  ...
  this.WOW = (function() {
    class WOW {
      ...
      start() {
        ...
        if (this.boxes.length) {
          if (this.disabled()) {
            ...
            for (j = 0, len = ref.length; j < len; j++) {
              box = ref[j];
              this.applyStyle(box, false, true);
            }
          }
        }
        if (!this.disabled()) {
      ...
      applyStyle(box, hidden, cached) {
        ...
        return this.animate(() => {
          return this.customStyle(box, hidden, cached, duration, delay, iteration);
        });
      }
      ...
      customStyle(box, hidden, cached, duration, delay, iteration) {
        ...
        this.vendorSet(box.style, {
          animationName: cached ? 'none' : this.cachedAnimationName(box)
        });
        return box;
      }
